...
USING JQUERY IS NOT ALLOWED IN ANY CASE, however you can use bootstrap as much as you can an of course any native javascript function such as map, filter, etc.

0.We give you a JSON file with an array of objects (items.json). Get the data from that file in the controller using the $http angular service. If you don't know how to do this, no problem.
In the controller you got a commented out variable with the array, so you can use it and carry on with the exercise.
...
4.Add a dropdownlist above the grid which allows you to filter the items by the item type. The dropdownlist will display the 3 different Item Types "category1","category2" and "category3" and another option "All" to display all items when you select it.

5.Add a "DELETE" button on each item of the grid, so that when you click on it, the item is removed from the grid.
...
7.Now you have to reorder the items in the grid and apply different colour rules to them.
